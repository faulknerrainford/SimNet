...
with dri.session() as ses:
    ...
    ses.run("MATCH (a), (b) "
            ...
            "CREATE (a)-[r:REACHES { worth: -10, effort:0 , mobility:1, confidence:1, modm:-0.4, modc:-0.6}]->(b)")
    ses.run("MATCH (a), (b) "
    ...
    ses.run("MATCH (a) "
            "WHERE a.name = 'Ind' "
            "CREATE (a)-[r:REACHES { worth:0, effort:0, mobility:1, confidence:1, energy: 0, modm:-0.02}]->(a)")
    ses.run("MATCH (a), (b) "
            "WHERE a.name='Home' AND b.name='Ind' "
            "CREATE (a)-[r:REACHES {effort:1, worth:-1, mobility:1, confidence:1, energy: 0.2, modm:0, modc:0.4}]->(b)")
    ses.run("MATCH (a), (b) "
            "WHERE a.name='Home' AND b.name='Care' "
            "CREATE (a)-[r:REACHES {effort:1, worth:-1, mobility:1, confidence:1}]->(b)")
